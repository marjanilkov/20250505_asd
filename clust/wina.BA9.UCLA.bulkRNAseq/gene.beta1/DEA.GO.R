...
rownames(classification) = classification$sample
...
#saveRDS(deg, paste(tissue,"UCLA.b1.DEP.RDS", sep = ""))
degList = DEGmachine(deg, FC.cutoff = FC_cutoff)
################################################################################
...
G_list = readRDS("C:/Users/Marjan Ilkov/OneDrive - The Mount Sinai Hospital/Desktop/MSSM/2025/20250505_asd/data/UCLA-ASD/bulk_rnaSeq/UCLA_ASD_bulk_rnaSeq.RDS")
G_list = G_list[,1:2]
enrichList = list()
...
for ( i in 1:length(tmp0))
{
  ...
  if (length(tmp1)>0)
  {
    ...
    tmp1 = merge(tmp1, G_list, by.x = "tmp1", by.y = "Geneid")
    tmp1 = tmp1[, c("Symbol","X.group.")]
...
#saveRDS(enrichList, paste(tissue, ".UCLA.BA9.bulkrnaseqGOterms.RDS", sep = ""))
